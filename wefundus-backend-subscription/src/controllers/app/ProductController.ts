...
class ProductController {
  ...
  async productSearch(req: ReqInterface, res: ResInterface, next: NextFunction) {
    try {
      ...
      const results = await SearchModel.find(match);
      return ResponseHelper.ok(res, res.__('search_results'), { results })
    } catch (error) {
      next(error);
    }
  }
  ...
  async productList(req: ReqInterface, res: ResInterface, next: NextFunction) {
    try {
      ...
      if (subcategory)
        match.subcategory = new Types.ObjectId(subcategory)

      if (section)
        match.section = new Types.ObjectId(section)
      const aggregate = [
        {
          '$match': match
        },
      ...
      return ResponseHelper.ok(res, res.__('product_list'), data[0]);
  ...
  async productListFilter(req: ReqInterface, res: ResInterface, next: NextFunction) {
    try {
      const queryObj = req.body;
      if (queryObj?.price) {
        const [min, max] = queryObj?.price;
        if (min > max)
          return ResponseHelper.badRequest(res, res.__('min_max_price'));
      }
      const data = await ProductService.productList(queryObj);
  ...
  async productDetails(req: ReqInterface, res: ResInterface, next: NextFunction) {
    try {
      const userId = req.query.userId as string;
      const productId = req.params.id as string;

      if (!(await ProductModel.findById(productId))) return ResponseHelper.badRequest(res, res.__('invalid_product_id'), { productId })
      const data = await ProductService.productDetails(productId, userId);
      res.logMsg = 'Product detail fetched successfully';
      ResponseHelper.ok(res, res.__('product_details'), data);
    } catch (error) {
      next(error);
    }
  }
