...
class CategoryController {
    ...
    /**
        ...
        *
        */
    async categoryList(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            ...
            return ResponseHelper.ok(res, res.__('category_list'), { categories });
        } catch (error) {
            next(error);
        }
    }
    ...
    async subcategoryListById(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            const categoryId = req.params.id;
            const isExists = await CategoryModel.exists({ _id: categoryId });
            if (!isExists) {
                return ResponseHelper.badRequest(res, res.__('invalid_category_id'), { categoryId });
            }
            ...
            return ResponseHelper.ok(res, res.__('subcategory_list'), { subcategories });
    ...
    async subcategoryListBySlug(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            const slug = req.params.slug;
            const category = await CategoryModel.findOne({ slug: slug });
            if (!category) {
                return ResponseHelper.badRequest(res, res.__('category_not_found'), { slug });
            }
            const subcategories = await SubcategoryModel.aggregate(
                [
                    {
    ...
    async categoryListAll(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
    ...
    async storeList(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            const aggregate = [
                {
                    '$match': {
            ...
            const stores = await CategoryModel.aggregate(aggregate);
            return ResponseHelper.ok(res, res.__('store_list'), { stores });
    ...
    */
    ...
    async sectionList(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            const subcategorySlug = req.params.subcategorySlug;
            const subcategory = await SubcategoryModel.findOne({ slug: subcategorySlug });
            if (!subcategory) {
                return ResponseHelper.badRequest(res, res.__('category_not_found'), { slug: subcategorySlug });
            }
            ...
            return ResponseHelper.ok(res, res.__('section_list'), { sections });
