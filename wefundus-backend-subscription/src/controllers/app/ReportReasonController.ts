...
class ReportReasonController {
    ...
    */
    ...
    async getReportReasonList(req: ReqInterface, res: ResInterface, next: NextFunction) {

        try {
            const categoryId = req.params.id;
            const reportReason = await ReportReasonModel.find(
                {
                    isDeleted: false,
                    categoryId,
                    isActive: true
            ...
            return ResponseHelper.ok(res, res.__('reportreason_list'), { reportReason })
