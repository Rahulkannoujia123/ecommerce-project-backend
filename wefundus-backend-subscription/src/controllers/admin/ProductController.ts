...
class ProductController {
    ...
    async add(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            ...
            if (product) {
                ...
                ResponseHelper.created(res, res.__('product_created'), { product });
            }
        } catch (error) {
            next(error);
        }
    ...
    */
    async list(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            ...
            const data = await ProductService.list(queryString);

            if (data) {
                res.logMsg = 'Product list fetch successfully';
                return ResponseHelper.ok(res, res.__('product_list'), data);
            }
        } catch (error) {
            next(error);
        }
   ...
   */
    ...
    async uploadImage(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            ...
            if (!coverPhoto && !productData.coverPhoto)
                return ResponseHelper.badRequest(res, res.__('cover_photo_required'));

            const product = await ProductService.uploadImage(productId, photos, coverPhoto, productData);
            if (product) {
                res.logMsg = 'Product image uploaded successfully';
                return ResponseHelper.created(res, res.__('photo_uploaded'), { product });
            }
            else {
    ...
    async changeCoverImage(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            console.log(req.files.coverPhoto)
            const productId = req.params.id;
            const coverPhoto = req.files.coverPhoto;
            if (!coverPhoto)
                return ResponseHelper.badRequest(res, res.__('coverphoto_is_required'));

            const product = await ProductService.changeCoverPhoto(coverPhoto, productId);
            if (product) {
                res.logMsg = 'Product coverPhoto uploaded successfully';
                return ResponseHelper.ok(res, res.__('coverPhoto_uploaded'), { product });
            }
            else {
                return ResponseHelper.serverError(res, res.__('s3_error'))
            }
    ...
    async edit(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            ...
            const product = await ProductService.edit(productData, req.params.id);
            if (product) {
                res.logMsg = 'Product edited successfully';
                ResponseHelper.ok(res, res.__('product_edited'), { product });
            }
        } catch (error) {
            next(error);
        }
    ...
    async delete(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            ...
            if (product) {
                ...
                ResponseHelper.ok(res, res.__('product_deleted'), { product });
            }
        } catch (error) {
            next(error);
        }
    ...
    async deleteCoverImage(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            console.log('req.params', req.params);
            const productId = req.params.id;
            const coverPhoto: any = req.query.coverPhoto
            const product = await ProductService.deleteCoverImage(productId, coverPhoto);
            if (product) {
                res.logMsg = 'Product deleted successfully';
                ResponseHelper.ok(res, res.__('product_deleted'), { product });
            }
    ...
    async deletePhoto(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            console.log('req.params', req.params);
            const productId = req.params.id;
            const photos: any = req.query.photos
            const product = await ProductService.deletePhoto(productId, photos);
            if (product) {
    ...
    async fetchProduct(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            ...
            if (data) {
                ...
                return ResponseHelper.ok(res, res.__('product_details'), data);
            }
        } catch (error) {
            next(error);
        }
   ...
   */
    async activeupdateStatus(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            ...
            return ResponseHelper.ok(res, res.__('product_change_status'), Product);
        } catch (error) {
            next(error)
        }
    };
   ...
   * */
    ...
    async addAttributes(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            ...
            attributes.forEach((e: Attribute) => {
                e.values.forEach((item: string) => {
                    if (!attributeValues.includes(item)) attributeValues.push(item);
                });
            });

            product.attributeValues = attributeValues;
            await product.save();
            await FilterService.updateFilter(product.categoryId, { attributeList: product.attributes });
            res.logMsg = `Attributes added successfully for product *${productId}* `;
            return ResponseHelper.ok(res, res.__('attributes_added'), { product });
