...
import SubgroupService from '../../services/admin/SubgroupService';
class SubgroupController {
    ...
    async subgroupList(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            ...
            const query = SubgroupModel.find({ isDeleted: false });
            const countQuery = SubgroupModel.find({ isDeleted: false });
            ...
            return ResponseHelper.ok(res, res.__('subgroup_list'), { list, count });
    ...
    async subgroupDetails(req: ReqInterface, res: ResInterface, next: NextFunction) {
        try {
            ...
            const exist = await SubgroupModel.exists({ _id: subgroupId });

            if (!exist) {
                return ResponseHelper.badRequest(res, res.__('subgroup_does_not_exist'));
            }
            const subgroup = await SubgroupService.subgroupDetails(subgroupId);
            return ResponseHelper.ok(res, res.__('subgroup_details'), { subgroup });
