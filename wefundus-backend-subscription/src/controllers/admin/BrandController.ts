...
class BrandController {
  ...
  async addBrand(req: ReqInterface, res: ResInterface, next: NextFunction) {
    try {
      ...
      if (!logoImage)
        return ResponseHelper.badRequest(res, res.__("brand_logo_required"));

      if (await BrandModel.exists({ name }))
        return ResponseHelper.conflict(res, res.__('brand_already_exists'));

      const data = await BrandService.addBrand(name, logoImage, categories);
      if (data) {
        res.logMsg = "Brand added successfully";
        return ResponseHelper.created(res, res.__("brand_added"), data);
      } else {
        return ResponseHelper.serverError(res, res.__("server_error"));
      }
    } catch (error) {
      next(error);
    }
  ...
  async list(req: ReqInterface, res: ResInterface, next: NextFunction) {
    try {
      ...
      const data = await BrandService.list(queryString);

      res.logMsg = "Brand list fetch Successfully";
      return ResponseHelper.ok(res, res.__("brand_list"), data);
    } catch (error) {
      next(error);
    }
  }
  ...
  async update(req: ReqInterface, res: ResInterface, next: NextFunction) {
    try {
      ...
      const brand = await BrandService.edit(id, logoImage, name, categories);
      if (brand) {
        res.logMsg = "Brand updated successfully";
        ResponseHelper.ok(res, res.__("brand_updated"), { brand });
      }
    } catch (error) {
      next(error);
    }
  ...
  async ChangeBrandStatus(
    ...
    try {
      ...
      let brandData = await BrandModel.findById(brandId);
      ...
      return ResponseHelper.ok(res, res.__("brand_changed_status"), {});
    } catch (error) {
      next(error);
    }
  }
