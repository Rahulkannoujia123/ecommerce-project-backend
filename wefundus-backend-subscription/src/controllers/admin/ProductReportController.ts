...
import ProductReportService from "../../services/admin/ProductReportService";
...
class ProductReportController {
  ...
  async list(req: ReqInterface, res: ResInterface, next: NextFunction) {
    try {
      ...
      const data = await ProductReportService.list(queryString);
      
      if (data) {
        res.logMsg = `Product Report list fetched Successfully`;
        return ResponseHelper.ok(res, res.__("product_report_list"), data );
      }
    } catch (error) {
      next(error);
    }
